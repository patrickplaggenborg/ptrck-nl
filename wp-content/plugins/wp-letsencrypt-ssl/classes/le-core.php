...
class WPLE_Core
{
    ...
    )
    {
        ...
        if ( !empty($opts) ) {
            ...
            $this->email = ( isset( $optss['email'] ) ? $optss['email'] : '' );
            $this->date = ( isset( $optss['date'] ) ? $optss['date'] : '' );
        }
        
        $this->basedomain = str_ireplace( array( 'http://', 'https://' ), array( '', '' ), site_url() );
    ...
    {
        try {
            $this->client = new WPLEClient(
                $this->email,
                false,
                WPLEClient::LOG_STATUS,
                ABSPATH . 'keys/'
            );
        } catch ( Exception $e ) {
            $this->wple_log(
        ...
        try {
            ...
            if ( !empty($this->pendings) && $http == 1 ) {
                ...
                foreach ( $this->pendings as $chlng ) {
                    $opts['challenge_files'][] = $chlng['filename'];
                }
        ...
        if ( !$this->order->allAuthorizationsValid() ) {
            ...
            if ( !empty($this->pendings) ) {
                ...
                foreach ( $this->pendings as $challenge ) {
                    ...
                    if ( $challenge['type'] == 'dns-01' && stripos( $challenge['identifier'], $site ) >= 0 ) {
    ...
    private function reloop_get_dns( $return = false )
    {
        ...
        foreach ( $this->pendings as $challenge ) {
            ...
            if ( $challenge['type'] == 'dns-01' && stripos( $challenge['identifier'], $site ) >= 0 ) {
                $vrfy .= 'Name: <b>_acme-challenge.' . $site . '</b>
          TTL: <b>60</b>
          Type: <b>TXT</b>
        ...
        if ( isset( $getopts['challenge_files'] ) ) {
            if ( !empty($getopts['challenge_files']) ) {
                foreach ( $getopts['challenge_files'] as $chfile ) {
                    $cfile = ABSPATH . sanitize_text_field( $chfile );
    ...
    private function wple_try_symlink_restore( $challenge, $acmefile )
    {
        ...
        $abspath = ABSPATH;
        @symlink( $abspath . $challenge['filename'], $acmefilepath );
        $rsponse = $this->wple_get_file_response( $acmefile );
                        ...
                        if ( isset( $arr['error'] ) && $arr['type'] == 'http-01' ) {
                            ...
                            if ( $sp != FALSE ) {
                                ...
                                if ( $fname != '' ) {
                                    ...
                                    @file_put_contents( $chpath, $wrong_code );
                                    @file_put_contents( $abpath, $wrong_code );
                                    $this->wple_log( sprintf( esc_html__( 'Updated challenge file %s with %s', 'wp-letsencrypt-ssl' ), $fname, $wrong_code ), 'success', 'a' );
                                }
